...
// ─── BANCO DE CRUZAMENTO ──────────────────────────────────────────
// Salva automaticamente os dados em índices para cruzamento futuro
...
// Busca CAR no banco pelo CCIR, ITR, nome ou proprietário
async function buscarCARnoBanco(tipo, valor) {
  try {
    ...
    // Busca parcial para nome e proprietário
    if (tipo === "nomeFazenda" || tipo === "proprietario") {
      const prefixo = chave.substring(0, 10);
      const q = await db.collection(colecao)
        .where("__name__", ">=", prefixo)
        .where("__name__", "<=", prefixo + "\uf8ff")
        .limit(1).get();
...
// Busca dados completos do banco interno pelo CAR
...
// Pré-popula a Fazenda Boa Esperança no banco se ainda não existir
async function garantirDadosIniciais() {
  try {
    const db = getDB();
    const carNorm = "MA2111763OFDDE447C03F432289FB78C642A3E4C2";
    // Usa a chave normalizada real
    const carNormReal = norm("MA-2111763-0FDDE447C03F432289FB78C642A3E4C2");
    const snap = await db.collection("banco_imoveis").doc(carNormReal).get();
    if (snap.exists) return; // já existe

    const batch = db.batch();
    const registro = {
      car:          "MA-2111763-0FDDE447C03F432289FB78C642A3E4C2",
      ccir:         "11003503617005",
      ccirFormatado:"110.035.036.170-5",
      itr:          null,
    ...
    batch.set(db.collection("banco_imoveis").doc(carNormReal), registro);
    batch.set(db.collection("indice_ccir").doc(norm("11003503617005")), { car: registro.car, carNorm: carNormReal });
    batch.set(db.collection("indice_nome").doc(norm("FAZENDA BOA ESPERANÇA").substring(0,30)), { car: registro.car, carNorm: carNormReal, nome: registro.nome });
    await batch.commit();
    console.log("[BANCO] Fazenda Boa Esperança cadastrada.");
  } catch (e) { console.log("[BANCO INIT] erro:", e.message); }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── ENRIQUECE COM BANCO INTERNO ───────────────────────────────
    const sicarFinal = enriquecerComBanco(sicar, dadosBanco, { ccir, itr });
    ...
    // Salva no cache e atualiza banco de cruzamento
...
// ─── ENRIQUECER COM BANCO INTERNO ────────────────────────────────
function enriquecerComBanco(sicar, banco, extras = {}) {
  if (!banco) return sicar;

  if (sicar?.encontrado) {
    return {
      ...sicar,
      ccir:         sicar.ccir         || banco.ccir          || extras.ccir || null,
      nirf:         sicar.nirf         || banco.itr           || extras.itr  || null,
      proprietario: sicar.proprietario || banco.proprietario  || null,
      nome:         sicar.nome         || banco.nome          || null,
      municipio:    sicar.municipio    || banco.municipio     || null,
    };
  }

  // SICAR não achou mas temos dados do banco — retorna o que temos
  if (banco.car) {
    return {
      encontrado:    true,
      fonteBanco:    true,
      car:           banco.car,
      nome:          banco.nome          || "Imóvel Rural",
      municipio:     banco.municipio     || "",
      uf:            banco.uf            || "",
      area:          banco.area          || null,
      ccir:          banco.ccir          || banco.ccirFormatado || extras.ccir || null,
      nirf:          banco.itr           || extras.itr          || null,
      proprietario:  banco.proprietario  || null,
      lat:           banco.lat           || null,
      lng:           banco.lng           || null,
      situacao:      "AT",
      situacaoLabel: "Ativo",
...
async function buscarSICAR({ car, ccir, itr, proprietario, nomeFazenda }) {
  try {
    ...
    if (ccir) {
      ...
      return { encontrado: false, mensagem: "CCIR não localizado no SICAR." };
    }
    ...
    if (itr) {
      ...
      return { encontrado: false, mensagem: "ITR/NIRF não localizado no SICAR." };
    }
    ...
    if (nomeFazenda) {
      ...
      return { encontrado: false, mensagem: `"${nomeFazenda}" não localizada.`, dica: 'Tente: "Nome - MA"' };
    }
    ...
    if (proprietario) {
      ...
      return { encontrado: false, mensagem: `"${proprietario}" não localizado.`, dica: 'Tente: "Nome - MA"' };
    }
...
async function buscarSIGEF({ car, ccir }) {
  ...
  try {
    ...
    return {
      encontrado: true, certificado: p.situacao==="CE",
      ...
      municipio: p.municipio_localizado, uf: p.uf,
