    ...
    for (const url of urls) {
      try {
        ...
        if (data && !data.erro) {
          return {
            fonte: "SNCR",
            nome: data.denominacao || data.nome || null,
            proprietario: data.proprietario || data.nome_proprietario || null,
...
async function detectarEstadoPorGPS(lat, lng) {
  try {
    const resp = await fetchComTimeout(
      ...
      { timeout: 4000, headers: { "User-Agent": "AgroMind/1.0" } }
    );
    if (!resp.ok) return null;
    const data = await resp.json();
...
// ─── 7. GPS → CAR (BBOX — só 1 tentativa, rápido) ───────────────
async function buscarCARporGPS(lat, lng) {
  const uf = await detectarEstadoPorGPS(lat, lng);
  if (!uf) return null;

  // Testa os 3 buffers em PARALELO (não sequencial) — muito mais rápido
  ...
  // Pega o primeiro que retornar resultado
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const { car, ccir, itr, lat, lng } = body;
