...
function dadosParaFazenda(dados) {
  ...
  const proprietario = dadosDocumento.proprietario || dados.sicar?.proprietario || dados.sigef?.denominacao || null;
  const nome = dadosDocumento.nomeFazenda || dados.sicar?.nome || dados.sigef?.denominacao || dados.car || "Imóvel Rural";
  const ccir = dadosDocumento.ccir || dados.sigef?.ccir || dados.sicar?.ccir || null;
  const itr = dadosDocumento.nirf || dados.sicar?.nirf || null;
  const matricula = dadosDocumento.matricula || null;
  return {
...
function calcularAreaHa(pontos) {
  ...
  let area = 0; const n = pontos.length;
  for (let i = 0; i < n; i++) {
    const j = (i + 1) % n;
    const xi = pontos[i][1]*Math.PI/180*6371000, yi = pontos[i][0]*Math.PI/180*6371000;
    const xj = pontos[j][1]*Math.PI/180*6371000, yj = pontos[j][0]*Math.PI/180*6371000;
    area += xi*yj - xj*yi;
  }
  return Math.abs(area/2)/10000;
}
function formatarDistancia(metros) { return metros<1000?`${metros.toFixed(1)} m`:`${(metros/1000).toFixed(3)} km`; }
function formatarArea(ha) { return ha<1?`${(ha*10000).toFixed(0)} m²`:`${ha.toLocaleString("pt-BR",{maximumFractionDigits:4})} ha`; }
...
function getInfoCampo(campo, fazenda, dadosReais) {
  const configs = {
    car:{ titulo:"📋 Código CAR", cor:C.accent, conteudo:()=>(<div><div style={{fontSize:11,color:C.textMuted,marginBottom:12}}>O CAR (Cadastro Ambiental Rural) é o registro eletrônico obrigatório para todos os imóveis rurais brasileiros (Lei 12.651/2012).</div><div style={{background:C.bg,borderRadius:8,padding:10,marginBottom:12,wordBreak:"break-all",fontSize:12,color:C.accentBright,fontFamily:"monospace"}}>{fazenda.car||"—"}</div>{[["Situação",fazenda.situacao],["Município",fazenda.municipio],["Área Total",fazenda.area],["Módulos",fazenda.modulos]].map(([l,v])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${C.border}`,fontSize:11}}><span style={{color:C.textMuted}}>{l}</span><span style={{fontWeight:600,color:C.text}}>{v||"—"}</span></div>))}<div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:12}}><button onClick={()=>{navigator.clipboard?.writeText(fazenda.car||"");alert("CAR copiado!");}} style={{flex:1,padding:"8px 0",borderRadius:8,background:`${C.accent}20`,border:`1px solid ${C.accent}40`,color:C.accent,fontSize:12,fontWeight:600,cursor:"pointer"}}>📋 Copiar CAR</button><a href="https://consultapublica.car.gov.br/publico/imoveis/index" target="_blank" rel="noreferrer" style={{flex:1,padding:"8px 0",borderRadius:8,background:`${C.blue}20`,border:`1px solid ${C.blue}40`,color:C.blue,fontSize:12,fontWeight:600,textDecoration:"none",textAlign:"center"}}>🌐 SICAR Oficial ↗</a></div></div>)},
    ccir:{ titulo:"📄 CCIR", cor:C.blue, conteudo:()=>(<div><div style={{fontSize:11,color:C.textMuted,marginBottom:12}}>O CCIR (Certificado de Cadastro de Imóvel Rural) é emitido pelo INCRA. Obrigatório para venda, arrendamento e herança rural.</div><div style={{background:C.bg,borderRadius:8,padding:10,marginBottom:12,fontSize:13,color:C.blue,fontFamily:"monospace",textAlign:"center",fontWeight:700}}>{fazenda.ccir!=="—"?fazenda.ccir:"Não disponível no SICAR"}</div>{fazenda.ccir==="—"?(<><div style={{padding:"10px 12px",background:`${C.yellow}15`,border:`1px solid ${C.yellow}40`,borderRadius:8,fontSize:11,color:C.yellow,marginBottom:8}}>⚠️ O SICAR não publica o CCIR para este imóvel. Faça upload do documento CCIR abaixo para preencher automaticamente.</div></>):(<button onClick={()=>{navigator.clipboard?.writeText(fazenda.ccir);alert("CCIR copiado!");}} style={{width:"100%",padding:"8px 0",borderRadius:8,background:`${C.blue}20`,border:`1px solid ${C.blue}40`,color:C.blue,fontSize:12,fontWeight:600,cursor:"pointer",marginBottom:8}}>📋 Copiar CCIR</button>)}<a href="https://sncr.serpro.gov.br/" target="_blank" rel="noreferrer" style={{display:"block",padding:"8px 0",borderRadius:8,background:`${C.blue}10`,border:`1px solid ${C.blue}30`,color:C.blue,fontSize:12,fontWeight:600,textDecoration:"none",textAlign:"center"}}>🌐 Consultar INCRA ↗</a></div>)},
    itr:{ titulo:"💰 ITR / NIRF", cor:C.yellow, conteudo:()=>(<div><div style={{fontSize:11,color:C.textMuted,marginBottom:12}}>O ITR (Imposto Territorial Rural) é cobrado anualmente pela Receita Federal. O NIRF é o número de inscrição do imóvel.</div><div style={{background:C.bg,borderRadius:8,padding:10,marginBottom:12,fontSize:13,color:C.yellow,fontFamily:"monospace",textAlign:"center",fontWeight:700}}>{fazenda.itr||"Não informado"}</div>{fazenda.areaHa&&(<div style={{background:`${C.yellow}10`,border:`1px solid ${C.yellow}30`,borderRadius:8,padding:"10px 12px",marginBottom:8}}><div style={{fontSize:11,color:C.textMuted,marginBottom:4}}>📊 Estimativa ITR (Lei 9.393/96)</div><div style={{fontSize:13,fontWeight:700,color:C.yellow}}>~R$ {(fazenda.areaHa*2.2).toLocaleString("pt-BR",{minimumFractionDigits:2})} / ano</div><div style={{fontSize:10,color:C.textMuted,marginTop:2}}>Baseado em VTN médio estimado</div></div>)}<a href="https://www.gov.br/receitafederal/pt-br/assuntos/orientacao-tributaria/tributos/itr" target="_blank" rel="noreferrer" style={{display:"block",padding:"8px 0",borderRadius:8,background:`${C.yellow}10`,border:`1px solid ${C.yellow}30`,color:C.yellow,fontSize:12,fontWeight:600,textDecoration:"none",textAlign:"center"}}>🌐 Receita Federal — ITR ↗</a></div>)},
    proprietario:{ titulo:"👤 Proprietário", cor:C.purple, conteudo:()=>(<div><div style={{fontSize:11,color:C.textMuted,marginBottom:12}}>Dados do proprietário/possuidor conforme SICAR, SIGEF/INCRA ou documento enviado.</div><div style={{background:C.bg,borderRadius:8,padding:12,marginBottom:12,textAlign:"center"}}><div style={{fontSize:16,fontWeight:800,color:C.accentBright,marginBottom:4}}>{fazenda.proprietario!=="—"?fazenda.proprietario:"Não informado"}</div><div style={{fontSize:11,color:C.textMuted}}>{fazenda.municipio}</div></div>{fazenda.proprietario==="—"?(<div style={{padding:"10px 12px",background:`${C.yellow}15`,border:`1px solid ${C.yellow}40`,borderRadius:8,fontSize:11,color:C.yellow}}>⚠️ Não disponibilizado pelo SICAR. Faça upload do CCIR ou Matrícula para preencher automaticamente.</div>):(<button onClick={()=>{navigator.clipboard?.writeText(fazenda.proprietario);alert("Nome copiado!");}} style={{width:"100%",padding:"8px 0",borderRadius:8,background:`${C.purple}20`,border:`1px solid ${C.purple}40`,color:C.purple,fontSize:12,fontWeight:600,cursor:"pointer"}}>📋 Copiar Nome</button>)}</div>)},
    matricula:{ titulo:"📜 Matrícula", cor:C.orange, conteudo:()=>(<div><div style={{fontSize:11,color:C.textMuted,marginBottom:12}}>A Matrícula é o registro do imóvel no Cartório de Registro de Imóveis. Comprova a propriedade legalmente.</div><div style={{background:C.bg,borderRadius:8,padding:10,marginBottom:12,fontSize:13,color:C.orange,fontFamily:"monospace",textAlign:"center",fontWeight:700}}>{fazenda.matricula!=="—"?fazenda.matricula:"Não informado"}</div>{fazenda.matricula==="—"?(<div style={{padding:"10px 12px",background:`${C.yellow}15`,border:`1px solid ${C.yellow}40`,borderRadius:8,fontSize:11,color:C.yellow}}>⚠️ Faça upload da Matrícula (PDF) para preencher automaticamente.</div>):(<button onClick={()=>{navigator.clipboard?.writeText(fazenda.matricula);alert("Matrícula copiada!");}} style={{width:"100%",padding:"8px 0",borderRadius:8,background:`${C.orange}20`,border:`1px solid ${C.orange}40`,color:C.orange,fontSize:12,fontWeight:600,cursor:"pointer"}}>📋 Copiar Matrícula</button>)}</div>)},
    app:{ titulo:"💧 APP", cor:C.blue, conteudo:()=>{const areaHa=fazenda.areaHa,appHa=parseFloat((fazenda.app||"0").replace(/[^\d,]/g,"").replace(",","."))||null,pct=areaHa&&appHa?((appHa/areaHa)*100).toFixed(1):null;return(<div><div style={{fontSize:11,color:C.textMuted,marginBottom:12}}>Área de Preservação Permanente. Protege recursos hídricos, encostas e topos de morro (Lei 12.651/2012).</div><div style={{background:`${C.blue}10`,border:`1px solid ${C.blue}30`,borderRadius:10,padding:"14px",marginBottom:12,textAlign:"center"}}><div style={{fontSize:24,fontWeight:900,color:C.blue}}>{fazenda.app||"—"}</div>{pct&&<div style={{fontSize:12,color:C.textMuted,marginTop:4}}>{pct}% da área total</div>}</div>{[["Rios até 10m","30m de cada margem"],["Rios 10-50m","50m de cada margem"],["Nascentes","50m de raio"],["Topo de morro","1/3 superior"]].map(([l,v])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",fontSize:10,padding:"3px 0",borderBottom:`1px solid ${C.border}`}}><span style={{color:C.textMuted}}>{l}</span><span style={{color:C.blue,fontWeight:600}}>{v}</span></div>))}</div>);}},
    rl:{ titulo:"🌱 Reserva Legal", cor:C.accentBright, conteudo:()=>{const areaHa=fazenda.areaHa,rlHa=parseFloat((fazenda.rl||"0").replace(/[^\d,]/g,"").replace(",","."))||null,pct=areaHa&&rlHa?((rlHa/areaHa)*100).toFixed(1):null;return(<div><div style={{fontSize:11,color:C.textMuted,marginBottom:12}}>Vegetação nativa obrigatória. Percentual varia por bioma: 80% Amazônia, 35% Cerrado, 20% demais.</div><div style={{background:`${C.accentBright}10`,border:`1px solid ${C.accentBright}30`,borderRadius:10,padding:"14px",marginBottom:12,textAlign:"center"}}><div style={{fontSize:24,fontWeight:900,color:C.accentBright}}>{fazenda.rl||"—"}</div>{pct&&<div style={{fontSize:12,color:C.textMuted,marginTop:4}}>{pct}% da área total</div>}</div>{[["🌿 Amazônia","80%","#22c55e"],["🏜️ Cerrado","35%","#fbbf24"],["🌾 Demais","20%","#3b82f6"]].map(([b,p,c])=>(<div key={b} style={{display:"flex",justifyContent:"space-between",padding:"5px 8px",marginBottom:4,borderRadius:6,background:`${c}10`,border:`1px solid ${c}20`,fontSize:11}}><span style={{color:C.textMuted}}>{b}</span><span style={{fontWeight:700,color:c}}>{p}</span></div>))}<a href="https://www.car.gov.br" target="_blank" rel="noreferrer" style={{display:"block",marginTop:8,padding:"8px 0",borderRadius:8,background:`${C.accent}10`,border:`1px solid ${C.accent}30`,color:C.accent,fontSize:12,fontWeight:600,textDecoration:"none",textAlign:"center"}}>🌐 Verificar no SICAR ↗</a></div>);}},
    ibama:{ titulo:"⛔ Embargos IBAMA", cor:C.red, conteudo:()=>{const embargos=fazenda.embargos||dadosReais?.ibama?.embargos||[],temEmbargo=fazenda.embargo||embargos.length>0;return(<div><div style={{marginBottom:12}}><span style={{display:"inline-flex",alignItems:"center",gap:6,padding:"6px 14px",borderRadius:20,fontSize:12,fontWeight:700,background:temEmbargo?`${C.red}20`:`${C.accent}20`,color:temEmbargo?C.red:C.accent,border:`1px solid ${temEmbargo?C.red:C.accent}40`}}>{temEmbargo?`⛔ ${embargos.length} embargo(s) ativo(s)`:"✅ Sem embargos ativos"}</span></div>{embargos.length>0?embargos.map((e,i)=>(<div key={i} style={{padding:"10px 12px",marginBottom:8,background:`${C.red}08`,border:`1px solid ${C.red}30`,borderRadius:8}}><div style={{fontSize:12,fontWeight:700,color:C.red}}>Auto nº {e.numero||"—"}</div>{[["Tipo",e.tipo],["Data",e.data],["Área",e.area],["Status",e.status]].filter(([,v])=>v).map(([l,v])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",fontSize:10,padding:"2px 0"}}><span style={{color:C.textMuted}}>{l}</span><span style={{color:C.text}}>{v}</span></div>))}</div>)):(<div style={{textAlign:"center",padding:"16px 0"}}><div style={{fontSize:36,marginBottom:8}}>✅</div><div style={{fontSize:13,color:C.accent,fontWeight:700}}>Nenhum embargo encontrado</div><div style={{fontSize:11,color:C.textMuted,marginTop:4}}>Fonte: IBAMA em tempo real</div></div>)}<a href="https://ibama.gov.br" target="_blank" rel="noreferrer" style={{display:"block",marginTop:8,padding:"8px 0",borderRadius:8,background:`${C.red}10`,border:`1px solid ${C.red}30`,color:C.red,fontSize:12,fontWeight:600,textDecoration:"none",textAlign:"center"}}>🌐 Consultar IBAMA ↗</a></div>);}},
    prodes:{ titulo:"📡 PRODES/INPE", cor:C.orange, conteudo:()=>{const alertas=fazenda.alertasProdes||dadosReais?.prodes?.alertas||[],temAlerta=fazenda.prodes||alertas.length>0,areaTotal=alertas.reduce((a,f)=>a+(f.areaKm2||0),0);return(<div><div style={{fontSize:11,color:C.textMuted,marginBottom:12}}>O PRODES/INPE monitora o desmatamento por satélite. O DETER emite alertas em tempo real na Amazônia Legal.</div><div style={{marginBottom:12}}><span style={{display:"inline-flex",alignItems:"center",gap:6,padding:"6px 14px",borderRadius:20,fontSize:12,fontWeight:700,background:temAlerta?`${C.orange}20`:`${C.accent}20`,color:temAlerta?C.orange:C.accent,border:`1px solid ${temAlerta?C.orange:C.accent}40`}}>{temAlerta?`🔴 ${alertas.length} alerta(s) — ${areaTotal.toFixed(2)} km²`:"🌳 Sem alertas de desmatamento"}</span></div>{!temAlerta&&(<div style={{textAlign:"center",padding:"16px 0"}}><div style={{fontSize:36,marginBottom:8}}>🌳</div><div style={{fontSize:13,color:C.accent,fontWeight:700}}>Sem alertas DETER/PRODES</div></div>)}{alertas.map((a,i)=>(<div key={i} style={{padding:"8px 10px",marginBottom:6,background:`${C.orange}08`,border:`1px solid ${C.orange}30`,borderRadius:8}}><div style={{fontSize:12,fontWeight:700,color:C.orange}}>{a.classname||"Desmatamento"}</div><div style={{fontSize:10,color:C.textMuted}}>{a.data||"—"} · {a.areaKm2||"—"} km²</div></div>))}<a href="http://terrabrasilis.dpi.inpe.br" target="_blank" rel="noreferrer" style={{display:"block",marginTop:8,padding:"8px 0",borderRadius:8,background:`${C.orange}10`,border:`1px solid ${C.orange}30`,color:C.orange,fontSize:12,fontWeight:600,textDecoration:"none",textAlign:"center"}}>🌐 TerraBrasilis INPE ↗</a></div>);}},
    sigef:{ titulo:"🗂️ SIGEF/INCRA", cor:C.blue, conteudo:()=>{const certificado=dadosReais?.sigef?.certificado,s=dadosReais?.sigef;return(<div><div style={{fontSize:11,color:C.textMuted,marginBottom:12}}>O SIGEF do INCRA certifica o georreferenciamento. Obrigatório para regularização e registro em cartório.</div><div style={{marginBottom:12,textAlign:"center"}}><span style={{display:"inline-flex",alignItems:"center",gap:6,padding:"8px 16px",borderRadius:20,fontSize:13,fontWeight:700,background:certificado?`${C.accent}20`:`${C.yellow}20`,color:certificado?C.accent:C.yellow,border:`1px solid ${certificado?C.accent:C.yellow}40`}}>{certificado?"✅ Georreferenciamento Certificado":"⚠️ Não certificado no SIGEF"}</span></div>{s?.encontrado&&([["Denominação",s.denominacao],["Área Registrada",s.area],["Município",s.municipio],["UF",s.uf],["CCIR",s.ccir],["Código INCRA",s.codigoIncra]].filter(([,v])=>v).map(([l,v])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:`1px solid ${C.border}`,fontSize:11}}><span style={{color:C.textMuted}}>{l}</span><span style={{color:C.text,fontWeight:600,maxWidth:150,textAlign:"right"}}>{v}</span></div>)))}<a href="https://sigef.incra.gov.br" target="_blank" rel="noreferrer" style={{display:"block",marginTop:8,padding:"8px 0",borderRadius:8,background:`${C.blue}10`,border:`1px solid ${C.blue}30`,color:C.blue,fontSize:12,fontWeight:600,textDecoration:"none",textAlign:"center"}}>🌐 Consultar SIGEF ↗</a></div>);}},
    modulos:{ titulo:"📐 Módulos Fiscais", cor:C.yellow, conteudo:()=>{const m=parseFloat((fazenda.modulos||"0").replace(/[^\d,\.]/g,"").replace(",","."))||null,cl=!m?"—":m<1?"Minifúndio":m<=4?"Pequena Propriedade":m<=15?"Média Propriedade":"Grande Propriedade",cc=!m?C.textMuted:m<1?C.red:m<=4?C.accent:m<=15?C.yellow:C.orange;return(<div><div style={{fontSize:11,color:C.textMuted,marginBottom:12}}>Classifica o imóvel por tamanho relativo ao módulo fiscal do município (INCRA).</div><div style={{background:`${C.yellow}10`,border:`1px solid ${C.yellow}30`,borderRadius:10,padding:"14px",marginBottom:12,textAlign:"center"}}><div style={{fontSize:28,fontWeight:900,color:C.yellow}}>{fazenda.modulos||"—"}</div><div style={{fontSize:13,fontWeight:700,color:cc,marginTop:6}}>{cl}</div></div>{[["Minifúndio","< 1 módulo",C.red],["Pequena","1-4 módulos",C.accent],["Média","4-15 módulos",C.yellow],["Grande","> 15 módulos",C.orange]].map(([c,d,cor])=>(<div key={c} style={{display:"flex",justifyContent:"space-between",padding:"4px 8px",marginBottom:3,borderRadius:6,background:cl===c?`${cor}15`:"transparent",fontSize:11}}><span style={{color:cl===c?cor:C.textMuted,fontWeight:cl===c?700:400}}>{c}</span><span style={{color:C.textMuted,fontSize:10}}>{d}</span></div>))}</div>);}}
  };
      ...
      <div style={{position:"absolute",top:0,right:0,bottom:0,width:320,background:C.surface,borderLeft:`1px solid ${C.border}`,zIndex:1600,display:"flex",flexDirection:"column",transform:aberto?"translateX(0)":"translateX(100%)",transition:"transform 0.28s cubic-bezier(0.4,0,0.2,1)",boxShadow:aberto?"-8px 0 32px rgba(0,0,0,0.4)":"none"}}>
        <div style={{padding:"16px 16px 12px",borderBottom:`1px solid ${C.border}`,background:`linear-gradient(135deg,${C.orange}18,transparent)`,flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{fontSize:14,fontWeight:800,color:C.orange}}>🌾 Imóvel Vizinho</div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:16}}>
          {vizinho ? (
            <div>
              ...
              <div style={{background:`${C.orange}10`,border:`1px solid ${C.orange}30`,borderRadius:10,padding:12,marginBottom:12}}>
                {[["📋 CAR",vizinho.car?.length>24?vizinho.car.substring(0,24)+"...":vizinho.car],["🌾 Área",vizinho.area],["👤 Proprietário",vizinho.proprietario],["✅ Situação",vizinho.situacaoLabel],["📐 Módulos",vizinho.modulos],["📄 CCIR",vizinho.ccir],["💧 APP",vizinho.app],["🌱 Res. Legal",vizinho.rl]].filter(([,v])=>v&&v!=="—").map(([l,v])=>(
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={()=>{navigator.clipboard?.writeText(vizinho.car||"");alert("CAR copiado!");}} style={{flex:1,padding:"8px 0",borderRadius:8,background:`${C.orange}20`,border:`1px solid ${C.orange}40`,color:C.orange,fontSize:11,fontWeight:600,cursor:"pointer"}}>📋 Copiar CAR</button>
                <a href={`https://consultapublica.car.gov.br/publico/imoveis/index`} target="_blank" rel="noreferrer" style={{flex:1,padding:"8px 0",borderRadius:8,background:`${C.blue}20`,border:`1px solid ${C.blue}40`,color:C.blue,fontSize:11,fontWeight:600,textDecoration:"none",textAlign:"center"}}>🌐 SICAR ↗</a>
              </div>
            </div>
          ) : <div style={{textAlign:"center",padding:"24px 0",color:C.textMuted,fontSize:13}}>⏳ Carregando dados...</div>}
        </div>
      </div>
    </>
  );
...
function InfoRowClicavel({ label, value, campo, onClicar }) {
  ...
  return (
    <div onClick={()=>onClicar(campo)} onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
      ...
      <span style={{fontWeight:600,color:temValor?(hover?C.accentBright:C.text):C.textDim,textAlign:"right",maxWidth:140,transition:"color 0.15s"}}>{value??"—"}</span>
    </div>
  );
}
...
async function buscarSobreposicoes(lat, lng, car, map, L, onVizinhoClick) {
  ...
  const buffer = 0.12, bbox = `${lng-buffer},${lat-buffer},${lng+buffer},${lat+buffer}`;
  ...
  try {
    ...
    if (resp.ok) { const data = await resp.json(); (data.features||[]).forEach(f=>{ if(f.geometry)L.geoJSON(f,{style:{color:"#a78bfa",weight:2,fillColor:"#a78bfa",fillOpacity:0.2,dashArray:"6,4"}}).bindPopup(`<b style="color:#a78bfa">🏕️ Terra Indígena</b><br>${f.properties?.terrai_nom||"—"}<br><small>${f.properties?.fase_ti||""}</small>`).addTo(map); }); resultados.ti=data.features||[]; }
  } catch {}
  try {
    ...
    if (resp.ok) { const data = await resp.json(); (data.features||[]).forEach(f=>{ if(f.geometry)L.geoJSON(f,{style:{color:"#34d399",weight:2,fillColor:"#34d399",fillOpacity:0.2,dashArray:"8,4"}}).bindPopup(`<b style="color:#34d399">🌳 Unidade de Conservação</b><br>${f.properties?.nome_uc||"—"}<br><small>${f.properties?.categori3||""}</small>`).addTo(map); }); resultados.uc=data.features||[]; }
  } catch {}
  if (car) {
    try {
      ...
      if (uf) {
        ...
        const sicarUrl = `https://geoserver.car.gov.br/geoserver/sicar/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=sicar:sicar_imoveis_${uf}&CQL_FILTER=${encodeURIComponent(filtro)}&outputFormat=application/json&maxFeatures=10`;
        const resp = await fetch(`${PROXY_URL}?url=${encodeURIComponent(sicarUrl)}`, { signal:AbortSignal.timeout(12000) });
        if (resp.ok) {
          const data = await resp.json();
          (data.features||[]).forEach(f => {
            ...
            const prop = f.properties?.nom_proprietario||"—";
            const situacao = {AT:"Ativo",CA:"Cancelado",SU:"Suspenso"}[f.properties?.ind_status]||"Ativo";
            const layer = L.geoJSON(f, { style:{ color:"#f97316",weight:2,fillColor:"#f97316",fillOpacity:0.12,dashArray:"4,4" } });
            layer.bindPopup(`<div style="font-family:sans-serif;min-width:200px;padding:4px"><div style="font-weight:800;font-size:13px;color:#f97316;margin-bottom:6px">🌾 Imóvel Vizinho</div><div style="font-size:12px;font-weight:700;margin-bottom:2px">${nome}</div><div style="font-size:11px;color:#666;margin-bottom:2px">📍 ${f.properties?.nom_municipio||"—"}</div><div style="font-size:11px;color:#666;margin-bottom:2px">🌾 ${area} · ${situacao}</div><div style="font-size:11px;color:#666;margin-bottom:6px">👤 ${prop}</div><div style="font-size:10px;color:#999;word-break:break-all;margin-bottom:8px">${cod}</div><button onclick="window.dispatchEvent(new CustomEvent('agromind-vizinho',{detail:'${cod}'}))" style="width:100%;padding:6px 0;border-radius:6px;background:#f9731620;border:1px solid #f9731640;color:#f97316;font-size:11px;font-weight:600;cursor:pointer">📋 Ver dados completos</button></div>`);
...
function LaudoVisual({fazenda,dadosReais,numeroLaudo}){
  const score=dadosReais?.score,ibama=dadosReais?.ibama,prodes=dadosReais?.prodes,clima=dadosReais?.clima,nasa=dadosReais?.nasa;
  ...
  return(<div id="laudo-conteudo" style={{width:794,background:"#ffffff",fontFamily:"Georgia,serif",color:"#1a2e1a"}}>
    ...
    <div style={{padding:"28px 40px",display:"flex",flexDirection:"column",gap:24}}>
      <div><SH icon="📋" title="1. IDENTIFICAÇÃO DO IMÓVEL" color="#16a34a"/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 24px"}}><div><Row2 label="Nome da Fazenda" value={fazenda.nome}/><Row2 label="Município/UF" value={fazenda.municipio}/><Row2 label="Área Total" value={fazenda.area}/><Row2 label="Módulos Fiscais" value={fazenda.modulos}/><Row2 label="APP" value={fazenda.app}/><Row2 label="Reserva Legal" value={fazenda.rl}/></div><div><Row2 label="Proprietário" value={fazenda.proprietario}/><Row2 label="CAR" value={fazenda.car}/><Row2 label="CCIR" value={fazenda.ccir}/><Row2 label="ITR/NIRF" value={fazenda.itr}/><Row2 label="Matrícula" value={fazenda.matricula}/><Row2 label="SIGEF/INCRA" value={fazenda.sigef}/></div></div><div style={{marginTop:10}}><Row2 label="Latitude" value={`${fazenda.coordenadas?.lat}°`}/><Row2 label="Longitude" value={`${fazenda.coordenadas?.lng}°`}/></div></div>
      ...
      <div><SH icon="⛔" title="3. EMBARGOS IBAMA" color="#ef4444"/><div style={{marginBottom:10}}><Badge ok={!fazenda.embargo} textoOk="Sem Embargo Ativo" textoNok="Embargo IBAMA Ativo"/></div>{ibama?.embargos?.length>0?ibama.embargos.map((e,i)=>(<div key={i} style={{padding:"8px 12px",marginBottom:6,background:"#fef2f2",border:"1px solid #fca5a5",borderRadius:8,fontSize:11}}><div style={{fontWeight:700,color:"#dc2626"}}>⛔ Embargo #{i+1} — {e.numero||"—"}</div><div style={{color:"#6b7280"}}>Área: {e.area||"—"} · Data: {e.data||"—"} · {e.tipo||"—"}</div></div>)):<div style={{padding:"10px 14px",background:"#f0fdf4",border:"1px solid #86efac",borderRadius:8,fontSize:11,color:"#166534"}}>✅ Nenhum embargo ativo encontrado na base IBAMA.</div>}</div>
      <div><SH icon="📡" title="4. PRODES/INPE — DESMATAMENTO" color="#f97316"/><div style={{marginBottom:10}}><Badge ok={!fazenda.prodes} textoOk="Sem Alerta PRODES" textoNok="Alerta PRODES Ativo"/></div>{prodes?.alertas?.length>0?prodes.alertas.map((a,i)=>(<div key={i} style={{padding:"8px 12px",marginBottom:6,background:"#fff7ed",border:"1px solid #fdba74",borderRadius:8,fontSize:11}}><div style={{fontWeight:700,color:"#ea580c"}}>🔴 Alerta #{i+1} — {a.classname||"Desmatamento"}</div><div style={{color:"#6b7280"}}>Área: {a.areaKm2||"—"} km² · Data: {a.data||"—"}</div></div>)):<div style={{padding:"10px 14px",background:"#f0fdf4",border:"1px solid #86efac",borderRadius:8,fontSize:11,color:"#166534"}}>✅ Nenhum alerta de desmatamento detectado.</div>}</div>
      {(clima?.encontrado||nasa?.encontrado)&&(<div><SH icon="🌤️" title="5. CLIMA & NASA POWER" color="#3b82f6"/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>{clima?.encontrado&&(<div><div style={{fontSize:11,fontWeight:700,color:"#3b82f6",marginBottom:6}}>🌤️ Clima Atual</div>{[["Temperatura",`${clima.atual?.temperatura??"—"}°C`],["Umidade",`${clima.atual?.umidade??"—"}%`],["Chuva 30d",`${clima.precipTotal30d??"—"} mm`]].map(([l,v])=>(<Row2 key={l} label={l} value={v}/>))}</div>)}{nasa?.encontrado&&(<div><div style={{fontSize:11,fontWeight:700,color:"#a78bfa",marginBottom:6}}>🛰️ NASA POWER</div>{[["Radiação Solar",nasa.radiacaoSolar?`${nasa.radiacaoSolar} MJ/m²`:"—"],["Temp. Média",nasa.temperaturaMedia?`${nasa.temperaturaMedia}°C`:"—"],["Precipitação",nasa.precipitacaoMedia?`${nasa.precipitacaoMedia} mm/d`:"—"]].map(([l,v])=>(<Row2 key={l} label={l} value={v}/>))}</div>)}</div></div>)}
      <div style={{background:"#f8fdf8",border:"1px solid #86efac",borderRadius:10,padding:"14px 18px",fontSize:10,color:"#374151",lineHeight:1.7}}><div style={{fontWeight:700,color:"#166534",marginBottom:6,fontSize:11}}>📋 Declaração</div>Laudo gerado automaticamente pela plataforma AgroMind com base em dados públicos oficiais (SICAR/CAR, IBAMA, PRODES/INPE, SIGEF/INCRA, Open-Meteo, NASA POWER). As informações são de caráter informativo. Dados privados (CCIR, Matrícula, Proprietário) foram fornecidos pelo usuário via upload de documento. Data: {dataHoje()}.</div>
    </div>
...
export default function MapaPage({ dadosConsulta }) {
  ...
  // ── UPLOAD DE DOCUMENTO ────────────────────────────────────────
  const processarDocumento = async (file) => {
    if (!file) return;
    ...
    try {
      ...
      const dadosExtraidos = extrairDadosDoCCIR(textoExtraido);
      ...
      const camposEncontrados = Object.keys(dadosExtraidos).length;
      if (camposEncontrados > 0) {
        alert(`✅ Documento processado!\n${camposEncontrados} campo(s) encontrado(s):\n${Object.entries(dadosExtraidos).map(([k,v])=>`• ${k}: ${v}`).join("\n")}`);
      } else {
        alert("⚠️ Documento processado, mas não foi possível extrair dados automaticamente.\nDigite os dados manualmente nos campos.");
      }
    } catch (err) {
  ...
  const fecharPoligono = () => {
    ...
    for (let i = 0; i < pontos.length; i++) {
      ...
      perM += calcularDistanciaM(pontos[i][0],pontos[i][1],pontos[j][0],pontos[j][1]);
    }
    ...
    setResultadoMedicao({
      tipo:"area", valor:formatarArea(ha), ha,
      perimetro:formatarDistancia(perM),
  ...
  const adicionarMarcador = (map, L, lat, lng, dados) => {
    ...
    L.marker([lat,lng],{icon}).addTo(map).bindPopup(`<div style="font-family:sans-serif;min-width:220px"><div style="font-weight:800;font-size:14px;color:#0d5c2e;margin-bottom:6px">🌿 ${dados.sicar?.nome||dados.sigef?.denominacao||dados.car||"Imóvel Rural"}</div><div style="font-size:12px;color:#666;margin-bottom:2px">📍 ${dados.sicar?.municipio||dados.sigef?.municipio||""}</div><div style="font-size:12px;color:#666;margin-bottom:2px">🌾 ${dados.sicar?.area||dados.sigef?.area||"—"}</div><div style="font-size:12px;color:#666;margin-bottom:2px">👤 ${dadosDocumento.proprietario||dados.sicar?.proprietario||dados.sigef?.denominacao||"—"}</div><div style="font-size:12px;color:#666;margin-bottom:2px">📄 CCIR: ${dadosDocumento.ccir||dados.sigef?.ccir||dados.sicar?.ccir||"—"}</div><hr style="margin:8px 0;border-color:#eee"/><div style="font-size:11px;color:#22c55e;font-weight:700">Score: ${dados.score?.valor||"—"}/100 — ${dados.score?.nivel||"—"}</div></div>`).openPopup();
  };
  ...
  const desenharReal = (map, L, geometria, dados) => {
    ...
    if (car && center.lat && center.lng) {
      setTimeout(async () => {
        try {
          const resultado = await buscarSobreposicoes(center.lat, center.lng, car, map, L, abrirDrawerVizinho);
  ...
  return (
    <div className="mapa-container" style={{display:"flex",height:"calc(100vh - 64px)",overflow:"hidden"}}>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}.leaflet-popup-content-wrapper{max-width:260px!important}.leaflet-popup-content{font-size:11px!important;margin:8px 10px!important}@media(max-width:768px){.leaflet-popup-content-wrapper{max-width:200px!important}.leaflet-popup-content{font-size:10px!important;margin:6px 8px!important}.leaflet-popup{margin-bottom:30px!important}}@media(max-width:768px){.mapa-container{flex-direction:column!important;height:auto!important;overflow-y:auto!important;}.mapa-painel-esq{width:100%!important;border-right:none!important;border-bottom:1px solid #1e3a1e!important;}.mapa-centro{width:100%!important;height:72vw!important;min-height:260px!important;max-height:400px!important;flex:none!important;}.mapa-painel-dir{display:none!important;}.mapa-score-flutuante{display:none!important;}.mapa-status-flutuante{display:none!important;}.mapa-legenda-flutuante{display:none!important;}}`}</style>
      ...
      <div className="mapa-painel-esq" style={{width:295,background:C.surface,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",flexShrink:0,overflowY:"auto"}}>
        ...
        <div style={{padding:14,borderBottom:`1px solid ${C.border}`}}>
          ...
          <div style={{fontSize:13,fontWeight:800,color:C.accentBright,marginBottom:2}}>{fazenda.nome}</div>
          <div style={{fontSize:11,color:C.textMuted,marginBottom:6}}>📍 {fazenda.municipio}</div>
          {docUpload&&Object.keys(docUpload).length>0&&(
            <div style={{background:`${C.blue}10`,border:`1px solid ${C.blue}30`,borderRadius:8,padding:"6px 10px",marginBottom:8,fontSize:10,color:C.blue}}>
              📄 Documento carregado — {Object.keys(docUpload).length} campo(s) preenchido(s)
            </div>
          )}
          ...
          <InfoRowClicavel label="📜 Matrícula"       value={fazenda.matricula}   campo="matricula"    onClicar={setCampoPainel}/>
        ...
        <div style={{padding:14,borderBottom:`1px solid ${C.border}`}}>
          <div style={{fontSize:12,fontWeight:700,marginBottom:6}}>📁 Upload de Documentos</div>
          <div style={{fontSize:10,color:C.textMuted,marginBottom:8}}>Envie o CCIR, Matrícula ou ITR para preencher os campos automaticamente</div>
          <input type="file" accept=".pdf,.txt,.doc" ref={docRef} style={{display:"none"}} onChange={e=>processarDocumento(e.target.files[0])}/>
          <button onClick={()=>docRef.current?.click()} disabled={uploadandoDoc} style={{width:"100%",padding:"9px 12px",borderRadius:8,background:uploadandoDoc?C.border:`linear-gradient(135deg,${C.blue},#2563eb)`,border:"none",color:C.text,fontWeight:600,fontSize:12,cursor:uploadandoDoc?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6,marginBottom:6}}>
            {uploadandoDoc?<><div style={{width:12,height:12,border:`2px solid ${C.text}`,borderTopColor:"transparent",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>Processando...</>:"📄 Enviar CCIR / Matrícula / ITR"}
          </button>
          {docUpload&&Object.keys(docUpload).length>0&&(
            <div style={{background:`${C.accent}10`,border:`1px solid ${C.accent}30`,borderRadius:8,padding:"8px 10px"}}>
              <div style={{fontSize:10,fontWeight:700,color:C.accent,marginBottom:4}}>✅ Dados extraídos:</div>
              {Object.entries(docUpload).map(([k,v])=>(<div key={k} style={{fontSize:10,color:C.textMuted,padding:"2px 0"}}><span style={{color:C.text,fontWeight:600}}>{k}:</span> {v}</div>))}
              <button onClick={()=>{dadosDocumento={};setDocUpload(null);if(dadosReais){const f=dadosParaFazenda(dadosReais);if(f)setFazenda(f);}}} style={{width:"100%",marginTop:6,padding:"5px 0",borderRadius:6,background:`${C.red}15`,border:`1px solid ${C.red}30`,color:C.red,fontSize:10,fontWeight:600,cursor:"pointer"}}>✕ Limpar documento</button>
            </div>
          )}
          <div style={{fontSize:10,color:C.textDim,marginTop:6,fontStyle:"italic"}}>Formatos: PDF, TXT · Dados privados (nunca enviados ao servidor)</div>
        </div>
        ...
        <div style={{padding:14,borderBottom:`1px solid ${C.border}`}}>
          ...
          {sobreposicoes&&(<div style={{display:"flex",flexDirection:"column",gap:4}}>
            ...
            {(sobreposicoes.vizinhos?.length||0)>0&&<div style={{fontSize:10,color:C.textMuted,fontStyle:"italic"}}>💡 Clique nos polígonos laranjas para ver dados</div>}
          </div>)}
          {!sobreposicoes&&temDadosReais&&<div style={{fontSize:10,color:C.textDim,fontStyle:"italic"}}>Vizinhos carregam automaticamente após 1s</div>}
        </div>
        ...
        <div style={{padding:14,borderBottom:`1px solid ${C.border}`}}>
          ...
          <div style={{display:"flex",gap:6,marginBottom:8}}>
            <button onClick={()=>iniciarMedicao("distancia")} style={{flex:1,padding:"9px 4px",borderRadius:8,border:`1px solid ${modoMedicao==="distancia"?C.blue:C.border}`,background:modoMedicao==="distancia"?`${C.blue}20`:"transparent",color:modoMedicao==="distancia"?C.blue:C.textMuted,fontSize:11,fontWeight:modoMedicao==="distancia"?700:400,cursor:"pointer"}}>
              📏 {modoMedicao==="distancia"?"Medindo...":"Distância"}
            </button>
            <button onClick={()=>iniciarMedicao("area")} style={{flex:1,padding:"9px 4px",borderRadius:8,border:`1px solid ${modoMedicao==="area"?C.accent:C.border}`,background:modoMedicao==="area"?`${C.accent}20`:"transparent",color:modoMedicao==="area"?C.accent:C.textMuted,fontSize:11,fontWeight:modoMedicao==="area"?700:400,cursor:"pointer"}}>
              ✏️ {modoMedicao==="area"?"Desenhando...":"Desenhar Área"}
            </button>
          </div>

          {modoMedicao==="area"&&(<div style={{padding:"10px 10px",background:`${C.accent}12`,border:`1px solid ${C.accent}40`,borderRadius:8,marginBottom:8}}>
            <div style={{fontSize:11,color:C.accent,fontWeight:700,marginBottom:4}}>✏️ Clique no mapa para desenhar</div>
            <div style={{fontSize:10,color:C.textMuted,marginBottom:6}}>Pontos: {medicaoPontosRef.current.length} — sem limite, acompanha curvas</div>
            <div style={{display:"flex",gap:5}}>
              {medicaoPontosRef.current.length>=3&&<button onClick={fecharPoligono} style={{flex:1,padding:"6px 0",borderRadius:6,background:`${C.accent}20`,border:`1px solid ${C.accent}50`,color:C.accent,fontSize:10,fontWeight:700,cursor:"pointer"}}>✅ Fechar e medir</button>}
              {medicaoPontosRef.current.length>=1&&<button onClick={desfazerPonto} style={{padding:"6px 10px",borderRadius:6,background:`${C.yellow}15`,border:`1px solid ${C.yellow}40`,color:C.yellow,fontSize:10,cursor:"pointer"}}>↩ Desfazer</button>}
            </div>
          </div>)}

          {modoMedicao==="distancia"&&(<div style={{padding:"8px 10px",background:`${C.blue}12`,border:`1px solid ${C.blue}40`,borderRadius:8,marginBottom:8}}>
            <div style={{fontSize:11,color:C.blue,fontWeight:700,marginBottom:4}}>📏 Clique nos pontos para medir</div>
            <div style={{fontSize:10,color:C.textMuted,marginBottom:4}}>Pontos: {medicaoPontosRef.current.length}</div>
            <button onClick={desfazerPonto} style={{width:"100%",padding:"5px 0",borderRadius:6,background:`${C.yellow}15`,border:`1px solid ${C.yellow}40`,color:C.yellow,fontSize:10,cursor:"pointer"}}>↩ Desfazer último</button>
          </div>)}

          {resultadoMedicao&&(<div style={{padding:"12px",background:C.card,border:`1px solid ${resultadoMedicao.tipo==="area"?C.accent:C.blue}40`,borderRadius:10,marginBottom:6}}>
            <div style={{fontSize:10,color:C.textMuted,marginBottom:4}}>
              {resultadoMedicao.tipo==="area"?"✏️ Área desenhada":"📏 Distância total"}
              <span style={{color:C.textDim}}> · {resultadoMedicao.pontos} pontos</span>
            </div>
            <div style={{fontSize:22,fontWeight:900,color:resultadoMedicao.tipo==="area"?C.accent:C.blue,marginBottom:4}}>{resultadoMedicao.valor}</div>
            {resultadoMedicao.ha&&(<>
              <div style={{fontSize:10,color:C.textMuted}}>= {(resultadoMedicao.ha*10000).toFixed(0)} m²</div>
              {resultadoMedicao.perimetro&&<div style={{fontSize:10,color:C.textMuted,marginTop:2}}>📏 Perímetro: <strong style={{color:C.text}}>{resultadoMedicao.perimetro}</strong></div>}
              {resultadoMedicao.coordenadas&&<div style={{fontSize:10,color:C.textMuted,marginTop:2}}>📍 Centro: <strong style={{color:C.text}}>{resultadoMedicao.coordenadas}</strong></div>}
            </>)}
          </div>)}

          {(modoMedicao||resultadoMedicao)&&(
            <button onClick={pararMedicao} style={{width:"100%",padding:"6px 0",borderRadius:8,border:`1px solid ${C.red}40`,background:`${C.red}15`,color:C.red,fontSize:11,fontWeight:600,cursor:"pointer"}}>✕ Limpar tudo</button>
          )}
          {!modoMedicao&&!resultadoMedicao&&<div style={{fontSize:10,color:C.textDim,fontStyle:"italic",marginTop:4}}>Clique no polígono verde para ver área do CAR</div>}
        </div>
        ...
        <div style={{padding:14,borderBottom:`1px solid ${C.border}`}}>
          ...
          <button onClick={()=>leafletMap.current?.setView([fazenda.coordenadas.lat,fazenda.coordenadas.lng],14)} style={{width:"100%",padding:"7px 0",borderRadius:8,background:`${C.green1}60`,border:`1px solid ${C.borderLight}`,color:C.accentBright,fontSize:11,fontWeight:600,cursor:"pointer"}}>🎯 Centralizar no Mapa</button>
        </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,overflow:"hidden"}}>
        <div className="mapa-centro" style={{flex:1,position:"relative",display:"flex",flexDirection:"column",minWidth:0}}>
          ...
          <div className="mapa-legenda-flutuante" style={{position:"absolute",bottom:40,left:16,background:`${C.surface}ee`,backdropFilter:"blur(12px)",border:`1px solid ${C.border}`,borderRadius:12,padding:"10px 12px",zIndex:1000}}>
            ...
            {modoMedicao&&<div style={{display:"flex",alignItems:"center",gap:7,marginTop:4,fontSize:11}}><div style={{width:16,height:4,borderRadius:2,background:modoMedicao==="area"?C.accent:C.blue,flexShrink:0}}/><span style={{color:modoMedicao==="area"?C.accent:C.blue}}>Medição</span></div>}
          </div>
          ...
          {/* Resultado medição */}
          {resultadoMedicao&&(<div style={{position:"absolute",top:70,left:"50%",transform:"translateX(-50%)",background:`${C.surface}f0`,backdropFilter:"blur(12px)",border:`1px solid ${resultadoMedicao.tipo==="area"?C.accent:C.blue}60`,borderRadius:12,padding:"12px 20px",zIndex:1100,textAlign:"center",boxShadow:"0 4px 20px rgba(0,0,0,0.4)"}}>
          ...
          {/* Cursor medição */}
          {modoMedicao&&(<div style={{position:"absolute",bottom:100,left:"50%",transform:"translateX(-50%)",background:`${modoMedicao==="area"?C.accent:C.blue}20`,border:`1px solid ${modoMedicao==="area"?C.accent:C.blue}60`,borderRadius:20,padding:"6px 16px",zIndex:1000,fontSize:11,color:modoMedicao==="area"?C.accent:C.blue,fontWeight:600,pointerEvents:"none"}}>
            {modoMedicao==="area"?"📐 Clique nos vértices — mínimo 3 pontos":"📏 Clique nos pontos para medir"}
          </div>)}
      ...
      <div className="mapa-painel-dir" style={{width:230,background:C.surface,borderLeft:`1px solid ${C.border}`,padding:"16px 14px",flexShrink:0,overflowY:"auto"}}>
        ...
        <div style={{marginTop:20}}>
          ...
          {[["Área Total",fazenda.area,C.accent],["APP",fazenda.app,C.blue],["Reserva Legal",fazenda.rl,C.accentBright],["Módulos",fazenda.modulos,C.yellow]].map(([l,v,c])=>(<div key={l} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:4}}><span style={{color:C.textMuted}}>{l}</span><span style={{fontWeight:700,color:c}}>{v||"—"}</span></div><div style={{height:4,background:C.bg,borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:l==="Área Total"?"100%":"30%",background:`linear-gradient(90deg,${c}80,${c})`,borderRadius:2}}/></div></div>))}
        </div>
